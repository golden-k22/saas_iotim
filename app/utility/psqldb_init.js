...
module.exports = async function initDB() {    
    ...
    db.sequelize.query("CREATE TABLE IF NOT EXISTS alarms(Id SERIAL PRIMARY KEY, name VARCHAR(60), device_sn VARCHAR(20), \"group\" INT2,  alarm_type INT2, low_warning REAL, high_warning REAL, low_threshold REAL, high_threshold REAL, offline_time INT2, repeat int[], date_from timestamp without time zone, date_to timestamp without time zone, time_from TIME, time_to TIME, created_at timestamp without time zone, status INT2,tenant_id VARCHAR(255) NOT NULL );")
    ...
    db.sequelize.query("CREATE TABLE IF NOT EXISTS gateway(Id SERIAL PRIMARY KEY, name VARCHAR(60), imei VARCHAR(20),  type INT2, phone_number VARCHAR(20), remark VARCHAR(100), status INT2, created_at timestamp without time zone, updated_at timestamp without time zone,tenant_id VARCHAR(255) NOT NULL);")
